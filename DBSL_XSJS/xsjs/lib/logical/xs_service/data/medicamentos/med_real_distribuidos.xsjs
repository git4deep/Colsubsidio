...
function deleteContentData(mPeriod){
	...
	sql = "DELETE FROM \"logical.data::T_MED_REAL_DISTRIBUIDOS\" " + 
...
function insertRows(aObjects){
	...
	aObjects.forEach(function(mObject){
        ...
        let sql = "INSERT INTO \"logical.data::T_MED_REAL_DISTRIBUIDOS\" (FISCPER, GL_ACCOUNT, COSTCENTER, VALOR) VALUES (?,?,?,?) ;";
        ...
        if ( mObject.COSTCENTER ) {
            pstmt.setNString(3, mObject.COSTCENTER);
        } else {
            pstmt.setNull(3);
        }
...
function getMetadata(){
    ...
    let mService = {
        METHOD: "GET",
        ENTITY: "DBMED_FIGL15_DIST_CV.xsodata",
        SET: "Distribuidos?$filter=FISCPER ge '" + mPeriod + "'"
    };
